...
public:
  ...
  std::pair<double, double> inverse_kinematics(double linear_velocity, double yaw_rate) const {
    double left_wheel_velocity = (linear_velocity - (yaw_rate * wheel_seperation_ / 2.0)) / wheel_radius_;
    double right_wheel_velocity = (linear_velocity + (yaw_rate * wheel_seperation_ / 2.0)) / wheel_radius_;
  ...
  std::pair<double, double> forward_kinematics(double left_wheel_velocity, double right_wheel_velocity) const {
    ...
    double yaw_rate = (right_wheel_velocity - left_wheel_velocity) * wheel_radius_ / wheel_seperation_;
